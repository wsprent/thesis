...
Given an undirected graph $G = (V, E, c)$ with vertices $V$, terminal set $N \subseteq V$, edges $E$, and edge weights $c: E \to \RR$,
...
that is $v \in N$, as \textit{terminals}, and vertice which are not required to be part of a solution but are so anyway, that is
$v \in T \cap (V \setminus N)$, as \textit{Steiner points} or \textit{Steiner vertices}.
...
be all edges which span the cut defined by $S$. Then we can formulate
 ...
 Constraint (\ref{form:stp:cut:cut}) ensures that any feasible solution, $T$,  must span all terminals in $G$, by
...
Given a \textit{directed} graph, $G = (V, A, c)$, with vertices $V$, arcs $A$, a non-empty terminal set $N \subseteq V$,
a root terminal $r \in N$, and arc-weights $c : A \to \RR$, then a Steiner Aborescence, $T \subseteq A$, is an aborescence
...
\begin{tikzpicture}[auto, node distance=1.7 cm]
  ...
  \end{scope}
\end{tikzpicture}
\caption{Instance of the Steiner Aborescence Problem. Terminals are coloured black, Steiner Points are coloured white, and the root terminal
  has a blue outline.}
\label{fig:sap:01}
\end{figure}
...
with cost $c(T) = 13.5$. It is worth noting that since all arcs must point away from the root in an aborescence that while the path $c, b, r$  has
