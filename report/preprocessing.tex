...
\paragraph{Example} Finally, we'll give an example of the power of
...
to solve the -- admittedly simplistic -- instance of the PCSTP
...
\draw (1) edge[snake it] node[snake node] {$NTD_1$} (2);
\draw (2) edge[snake it] node[snake node] {$TD_1$} (3);
\draw (3) edge[snake it] node[snake node] {$MA$} (4);
\draw (4) edge[snake it] node[snake node] {$UV$} (5);
\draw (5) edge[snake it] node[snake node] {$NTD_2$} (6);
\draw (6) edge[snake it] node[snake node] {$NTD_2$} (7);
\draw (7) edge[snake it] node[snake node] {$TD_2 + UV$} (8);
\end{tikzpicture}
\caption{Graph reduction on the PCSTP instance in Figure \ref{fig:pcstp:01}. In the final graph,
