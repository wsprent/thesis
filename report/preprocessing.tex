...
\begin{proof}
  ...
  edges
  in $E_T$. An example of this partition of $T$ is shown in Figure (\todo[inline]{reference}).
  Clearly, the total edge costs of these paths cannot be greater than the edge cost of $T$,
  that is, we have
  \begin{equation}
...
\end{equation}
  ...
  Finally, for the set of terminals not covered by $T$, we must have
  \begin{equation}
  ...
  \begin{tikzpicture}[auto, node distance=1.3cm]
   ...
   \begin{scope}
     ...
     \draw[selected, green] (vlll) edge (vll);
     \draw[selected, yellow] (vibr) edge (vib);

     \draw[selected, black] (vll) edge (vl);
     
     \draw[selected] (vj) edge (vi);
