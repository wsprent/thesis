...
First we will give an overview of the dataset. This is then
...
the benchmark suite had been run, we have decided to keep the data set.
 ...
 \item Default settings (\texttt{-{}-max-cuts 0}).
...
custom primal heuristic only sometimes being worthwhile mirrors results from
...
performance. If the heuristic was re-implemented to be blazingly fast, it may be advisable to
...
that a user cut routine must not just justify itself in terms of time spent versus tighter bounds,
