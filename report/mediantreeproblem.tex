...
\subsection{Results}
\textit{Benchmark suite}

\textit{Time taking}

\textit{Settings -- defaults and otherwise}

\textit{Ordering -- specific and what is not}
...
\textit{- LP relaxation/vs problemspecfic / search algorithms}

\textit{Maybe MTP is too generic?}

\textit{Gurobi: 5\% of the time vs us at every 50th node}
\begin{table}[h!]
  \centering
...
\textit{Gurobi is strongest in the majority of cases but not all}
\textit{Is the Python implementation just not strong enough?}
\paragraph{User Cuts}

\textit{Separation is costly -- more so with the LP relaxation}
\textit{Lower bounds should give more value}
\textit{Turns PreCrush}
\begin{table}[h!]
  \centering
...
\textit{Terrible performance for our user cuts.}
\paragraph{Strengthening the Model}
\textit{Size of model vs. size of feasible region (for the LP relaxation)}
\textit{Invariant of implementation interface}
...
\textit{Inconclusive: Strengthened model performs better on some instances - no clear tendencies}
\section{Conclusion}

\textit{Perhaps tests not timing-related are interesting?}
