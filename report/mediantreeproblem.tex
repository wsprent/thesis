...
our primal heuristic is twice as slow as the Gurobi heuristic.
...
In both cases, running the heuristic on every 25th node is not a very sophisticated way of using
our resources.

\textit{Is the Python implementation just not strong enough?}
\textit{Maybe be more sophisticated with when the heuristics are run}
\paragraph{User Cuts}
...
\begin{table}[h!]
  ...
  \end{tabular}
  \caption{Res}\label{tab:jmp:maxcut}
\end{table}
