...
methods used to solve the \gls{pcstp} to the MTP may result in good performance.
...
work done on the \gls{pcstp}. Finally, we will present a dataset for the MTP generated from a subset
...
Instances of the \gls{pcstp} can straightforwardly be reduced to instances of the MTP. This is in exact correspondence to how instances
...
$$d_{ij} =
 ...
 $$
 then solving the MTP on the graph $G' = (V, E, c, d)$ is equivalent to solving the \gls{pcstp} on $G$. Since every vertex assignment not
 ...
 any solution $T = (V, E, c, p)$ to the \gls{pcstp} on $G$ will have the exact same cost as the solution $T' = (V, E, c, d)$ to the MTP on $G'$.
