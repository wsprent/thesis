...
Then given a solution, $SA = (V_{SA}, A_{SA}, c')$,
to the transformed problem, we can generate a solution, $T$, to the
original problem by simply taking
the edges in $G$ which correspond to arcs in $SA$; the selected terminals are the all terminals
 which have in-degree higher than zero in $SA$.
However, it doesn't take much
investigation to see that if we simply find the connected subgraph in $G_A$ which minimises
$$c(SA) = \sum_{a \in A_{SA}} c'_{a} + \sum_{v \in N} p_v$$
we won't necessarily get an aborescence which will directly
correspond to a single, optimal prize collecting steiner tree in the original problem.

Firstly, if the out-degree of the root vertex isn't constrained, then all arcs orgininating in the root are
trivially part of any optimal solution, which will result in disconnected solutions in $G$.
Secondly, by representing vertex prizes within the cost of arcs,
...
\citet{ljubic2005solving} solve this by adding extra constraints to the problem. Firstly, the
...
in the original problem. Enforcing that any solution to the transformed problem must be an aborescence
 ...
 To ensure that exists a one-to-one relationship between solutions in $G_{SA}$ and $G$, only the selected 
 terminal
 ...
 \begin{figure}[h!]
  ...
  With these extra constraints in places, any feasible solution in the transformed, directed graph $G_{A}$ will correspond
  ...
  solutions to corresponding instances of the constrained SAP and the PCSTP. Figure \ref{fig:exact:sap:opt} shows this for our
   ...
   $$c(SA) = \sum_{ij \in A_{SA}} c'_{ij} + \sum_{v \in V} pv = (-12 + 4 -5) + (12 + 10 + 3) = 12$$
...
picking an arc which goes from the root to a vertex in $v' \in N_A$ incurs the penalty for not
