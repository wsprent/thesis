...
Solutions to the PCSTP were first explored by \cite{Bienstock1993} which proposed a 3-approximation algorithm based on Christophides algorithm for the TSP.
This was later improved to a primal-dual 2-approximation algorithm by \cite{goemans1997primal}, which was refined and implemented
for faster runtimes and typically better
bounds by \cite{Johnson:2000:PCS:338219.338637}.

\cite{canuto2001local} presented a primal heuristic based on multi-start local search with pertubations and path relinking.
...
presented a memetic algorithm using tournament selection and mutation. They additionally presented an ILP formulation of the Steiner
 ...
 \cite{ljubic2005solving} proposed the first full framework for solving the PCSTP to optimality. This was done by solving
 equivalent SAP instances through branch-and-cut where the LP-relaxation of a cut-formulation of the SAP is solved to generate
  a lower bound and a separation procedure is used to tighten this bound.   
  Furthermore, they presented a set of real-world problem instances for the PCSTP based on network layout in Berlin.

  While \cite{lucena2004strong}, \cite{Ljubic:2004:memetic}, and \cite{ljubic2005solving} applied preprocessing routines
  -- known as reduction tests --
  which were originally generalised for
  the Steiner Tree Problem by \cite{duin1989edge,duin1989reduction} for reducing graph sizes,
  these tests weren't stated and
  proved natively for the PCSTP until \citeauthor{uchoa2006reduction} did so in \citeyear{uchoa2006reduction}.
  \citeauthor{uchoa2006reduction} both restated and proved the validity of reduction tests which were direct restatements of tests defined for the STP,
  and also introduced a new
  novel way of determining distances between nodes, allowing for stronger reduction tests.

  The 11th DIMACS challenge on Steiner tree problems\footnote{\url{http://dimacs11.zib.de/}} brought not only a new set of instances for the PCSTP, but
  also a new wave of algorithms.

  Solving the PCSTP by transforming instances to directed variants was revisited by \cite{leitner2016dual} and \cite{gamrath2017scip}.
  \cite{leitner2016dual} transformed instances into the Asymmetric Prize-Collecting Steiner Tree Problem (APCSTP) and employed a
  branch-and-bound framework using a dual ascent method originally presented by \cite{wong1984dual} -- for the SAP -- for lower bounds and
  a shortest-path primal heuristics was used for finding good incumbents.
  ...
  This applied preprocessing routines natively to the PCSTP instances before transforming them into SAP instances. The SAP was formulated as an
  ILP in a so-called \textit{Flow-Balanced Directed Cut Formulation}. The general branch-and-bound procedures of the SCIP optimization suite were
